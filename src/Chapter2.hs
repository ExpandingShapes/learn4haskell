...
takeEven list = go list []
  where
    ...
    go list newList = 
      case length list of
        0 -> reverse newList
        1 -> reverse (list ++ newList)
        _ -> go (drop 2 list) (head list : newList)
...
-}
contains :: Eq a => a -> [[a]] -> [[a]]
contains x l = filter(\list -> x `elem` list) l
...
rotate n list
  | n < 0 = []
  | n == 0 = list
  | n > 0 = take (length list) (drop n (cycle list))
...
-}
rewind :: [a] -> [a]
rewind list = case list of
  [] -> []
  _ -> last list : rewind (init list)
